...
export default function Analytics({ expenses, categories }) {
    ...
    <div className="space-y-6 md:space-y-8">
      <div className="bg-amber-50 border border-amber-200 rounded-3xl p-5 md:p-6 flex flex-col sm:flex-row items-start gap-4">
        ...
        <div>
          ...
          <p className="text-amber-800 mb-3 text-xs md:text-sm">You've spent <strong>${variableTotal.toLocaleString()}</strong> on variable expenses. Cutting down on these is the fastest way to fund your savings goal!</p>
          <div className="flex flex-wrap gap-2">
            {variableItems.slice(0, 3).map(item => (
              <span key={item.name} className="bg-white px-2 py-1 md:px-3 md:py-1 rounded-full text-[10px] md:text-xs font-bold text-amber-700 shadow-sm border border-amber-100">{item.name}: ${item.amount.toLocaleString()}</span>
            ))}
          </div>
        </div>
      </div>
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-5 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mt-8">
          <div>
            ...
            <p className="text-2xl md:text-3xl font-bold mb-2">${fixedTotal.toLocaleString()}</p>
          ...
          <div>
            ...
            <p className="text-2xl md:text-3xl font-bold mb-4">${variableTotal.toLocaleString()}</p>
            <ul className="space-y-2">
              {variableItems.map(item => (
                <li key={item.name} className="flex justify-between text-xs md:text-sm border-b border-gray-50 pb-2"><span className="truncate pr-2">{item.name}</span><span className="font-bold text-gray-700 shrink-0">${item.amount.toLocaleString()}</span></li>
              ))}
            </ul>
          </div>
        </div>
